...
| `ros_backend/manipulation.py` | Arm pose sequences, the align-and-creep dock with its stall guard, and the magic-grasp pin | `align`, `grasp_sequence`, `back_away`, `detach` | `node` | **High.** Joint targets were measured on one robot; `DROP` and the reach constants are now solved from the URDF by `kinematics` |
...
| I17 | The pin holds the block where the pads closed on it, so grasping does not move it | `node.py`, `_teleport_pinned` + `CARRY_HOLD_BELOW_PADS` | Pinning to `gripper_base` jumped the block 0.133 m and parked it across the lidar plane, inflating a costmap obstacle over the robot's own payload |
